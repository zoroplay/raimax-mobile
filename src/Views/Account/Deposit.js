...
const Deposit = ({history}) => {
    ...
    const [activeTab, setActiveTab] = useState(    {slug: 'paystack', name: 'Paystack'});
    ...
    const getGateway = (gateway) => {
        ...
        getGatewayKeys(gateway.slug).then(res => {
            if (res.success) {
                ...
                dispatch({type: SET_LOADING_PROP, payload: {show: false, message: res.message}});
            }
        }).catch(err => {
            dispatch({type: SET_LOADING_PROP, payload: {show: false, message: err.message}});
        })
    }
                ...
                <div className="form">
                    ...
                    <div className="">
                        <div className="form-row">
                            <div className="form-label"><strong> Deposit Amount ({SportsbookGlobalVariable.Currency})</strong></div>
                            <div className="form-input">
                                <input name="amount"
                                       className="big" type="number"
                                       step="100" maxLength={5} min={SportsbookGlobalVariable.MinDeposit} max="10000"
                                       value={config.amount} onChange={(e) => setConfig({...config, amount: e.target.value})}
                                />
                                <div className="form-input--stake"> Min {SportsbookGlobalVariable.MinDeposit}</div></div>
                        ...
                        {
                            {
                                'monnify' : <button className="btn mt20 mb20 w-full" onClick={payWithMonnify}> Make Payment</button>,
