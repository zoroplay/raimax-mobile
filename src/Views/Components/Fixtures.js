...
import {formatDate, groupFixtures, isSelected, slugify} from "../../Utils/helpers";
            ...
            {fixtures && groupFixtures(fixtures).map((fixture, i) => (
                <div key={i}>
                    <div className="match-info table-f">
                        <div className="match-info--title">{fixture.event_date === today ? 'Today' : formatDate(fixture.event_date, 'ddd DD MMM, YYYY') }</div>
                        {predictions.length <= 3 && <div className="match-odds">
                            {predictions &&
                            <div className="table-f">
                                {predictions.map((prediction, i) =>  <div className="match-odds--value" key={i}>{prediction.odd_name}</div>)}
                            </div>}
                        </div>}
                    </div>
                    {fixture.events.map(match => (
                        ...
                        <div className={`match-content`}>
                            {showLeague && <div className="table-f">
                                <div className="match-content__row--league">
                                    {match.sport_category_name} - {match.sport_tournament_name}
                                </div>
                            </div>}
                            <div className={`${match.odds.length <= 3 ? 'table-a' : 'match-content-score'}`}>
                                <div className="match-content__info" id={`match_info_${match.provider_id}`}>
                                    <div className="match-content__row table-f">
                                        <div className="match-content__stats" onClick={() => toggleStat(match.provider_id)}>
                                            <div className={`match-content__stats-icon ${showStat === match.provider_id ? 'open' : ''}`} />
                                        </div>
                                    ...
                                    <div className="match-content__row table-f">
                                        <div className="match-content__row--info">
                                            <span><span>{match.event_time}</span> ● {match.markets_count}&nbsp;Markets</span>
                                        </div>
                                    </div>
                                </div>
                                {match.odds.length <= 3 ? (
                                    <div className="bets">
                                        <div className="bets__row table-f">
                                            {match.odds.map(odd => (
                                                <div
                                                    onClick={() => dispatch(addToCoupon(match, predictions[0]?.market_id, predictions[0].market_name, odd.odds, odd.id, odd.name, createID(match.provider_id, predictions[0]?.market_id, odd.name, odd.id), match.fixture_type))}
                                                    className={`bets__item ${(isSelected(createID(match.provider_id, predictions[0]?.market_id, odd.name, odd.id), coupon)) ? 'active' : ''}`}
                                                    key={odd.id}>
                                    ...
                                    <div className="match-scores new">
                                        {_.chunk(match.odds, 3).map((row, i) =>
                                            <div className="match-scores__row" key={`row-${i}`}>
                                                {row.map(odd =>
                                                    <div className="match-scores__item" key={`odds-${odd.id}`}>
                                                        <div className="table-f">
                        ...
                        {showStat === match.provider_id &&
                            ...
                            <div className="event-stats open">
                                ...
                                <div className="widgets">
                                    <div>
                                        <div className="sr-widget sr-widget-1" id={`sr-widget-${match.provider_id}`} />
                                    </div>
                                </div>
                            </div>
                                <div className="stats-toggle" onClick={() => setShowStat(null)}><i className="icon arrow" /></div>
                            </>
                        }
                        </div>
                    ))}
