...
export default function LiveFixtures({ activeSport }) {
  ...
  useEffect(() => {
    if (activeSport) {
      const sportMarket = LiveEventsOverview.find(
        (sport) => sport.id === activeSport.sport_id
      );
  ...
  return (
    <Fragment>
      {activeSport &&
        activeSport?.tournaments?.map((tournament) => (
          <div
            key={slugify(tournament.Name)}
            className="accordion-item league live-event open"
          >
            <div className="accordion-toggle live-event table-f">
              <div className="accordion-toggle__btn">{tournament.category} - {tournament.Name}</div>
            </div>
            <div className="accordion-content">
              <div className="accordion-inner">
                <div className="event-tips">
                  <div className="event-tips__holder">
                    {activeMarket?.outcomes?.map((selection) => (
                      <div
                        className="event-tips__holder-item"
                        key={selection.id}
                      >
                        {selection.name}
                      </div>
                ...
                {tournament.Events.map((match) => (
                  ...
                  >
                    <NavLink
                      to={`/liveEventDetail/${slugify(
                        activeSport.name
                      )}/${slugify(tournament.Name)}/${slugify(
                        match.event_name
                      )}/${match.provider_id}`}
                        ...
                        <div className="match-content__row--info">
                          <span>
                            <span>
                              ...
                              {match.live_data?.match_time
                                ? `, ${match.live_data?.match_time}'`
                                : ""}
                            </span>
                            &nbsp;● {match.live_data?.markets?.length || 0}
                            &nbsp;Markets
                          </span>
                        </div>
                      </div>
