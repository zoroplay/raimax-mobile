...
const Bonus = () => {
  ...
  return (
    <>
      <div className="bonus_text">Available Bonus Fund: {formatNumber(user?.sportBonusBalance)} NGN</div>
      <div className="bonus_text">Pending Bonus Fund: {formatNumber(pendingBalance)} NGN</div>
      ...
      {current === "active" ? (
        ...
        : <h4 className="center">No active bonus</h4>
      ) : (
        <div>
          {bonusHistory.length > 0  ?
              bonusHistory.map((data: any, idx: number) => <BetBonusAccordion key={`bonus-history-${idx}`} data={data} />)
            :
             <h4 className="center">No record found</h4>
          }
        </div>
      )}
    </>
