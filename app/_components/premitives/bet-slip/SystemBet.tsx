...
export const SystemBet = ({ coupon, dispatch, SportsbookBonusList }: SystemBetProps) => {
    ...
    const [defaultCombo, setDefaultCombo] = useState(coupon.combos[coupon.combos.length - 1]);
    ...
    const minMaxWin = (e: any, data: any, i: number) => {
        ...
        if (!coupondata.Groupings || !coupondata.Groupings.length) {
            ...
            const groupIndex = coupondata.Groupings.findIndex(function (g: any) { return g.Grouping === combo.Grouping; });
            if (groupIndex === -1) {
        ...
        const calculatedCoupon = couponCalculation.calcPotentialWins(coupondata, SportsbookBonusList, SportsbookGlobalVariable.MinBonusOdd);
        coupondata = couponCalculation.updateFromCalculatedCoupon(coupondata, calculatedCoupon, SportsbookGlobalVariable, SportsbookBonusList);
        let total = 0;
                ...
                <div className="input_stake start">
                    ...
                    <div className="input_stake_wrap">
                        <input 
                            ...
                            onFocus={handleFocus}
                            onChange={(e) => minMaxWin(e, defaultCombo, (coupon.combos.length - 1))}
                        />
                    </div>
                </div>
            </div>
            <div>
                {isCombiOpen && coupon.combos.length > 0 
                    && coupon.combos?.map((combo: any, idx: number) => 
                    ...
                    >
                        <div className="bet_slip_combi_text">
                            ...
                            {combo.checked === true && 
                            <div>
                                <small>{SportsbookGlobalVariable.Currency}{combo.minWin}/{SportsbookGlobalVariable.Currency}{combo.maxWin}</small>
                            </div>}
                        </div>
                        <div className="input_stake start">
                            <div
