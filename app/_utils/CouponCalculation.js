...
export default class CouponCalculation {
    ...
    calcCombinations = function (coupon) {
        ...
        var maxCombinationForCoupon = Math.min(process.env.NEXT_PUBLIC_MaxCombinationsByGrouping, process.env.NEXT_PUBLIC_MaxCouponCombinations);
    ...
    updateFromCalculatedCoupon = function (coupon, calculatedCoupon, globalVar, bonusList) {
        ...
        if (betCoupon.Groupings.length > 0) {
            for (var i = 0; i < calculatedCoupon.Groups.length; i++) {
                ...
                let betCouponGroup = betCoupon.Groupings.find(function (g) { return g.Grouping == calculatedGroup.Grouping; });
                if (betCouponGroup !== -1) {
    ...
    setPossibleCouponStake = function (betCoupon) {
        for (var _d = 0, _e = betCoupon.Groupings; _d < _e.length; _d++) {
            ...
            if (betCoupon.stake !== 0) {
                ...
                minInnerStakeAllowed = Math.max(process.env.NEXT_PUBLIC_MinGroupingsBetStake, (process.env.NEXT_PUBLIC_MinBetStake * grpStakeRatio));
            }
            if ((process.env.NEXT_PUBLIC_StakeInnerMod0Combination != null && process.env.NEXT_PUBLIC_StakeInnerMod0Combination !== 0)) {
                var modValueItem = grpItem.Stake % process.env.NEXT_PUBLIC_StakeInnerMod0Combination;
                //if correct then continue to next GroupItem
                if (modValueItem !== 0) {
                    //new Stake Value
                    var divValueItem = grpItem.Stake / process.env.NEXT_PUBLIC_StakeInnerMod0Combination;
                    var rounded = Math.round(divValueItem);
                    if (rounded === 0)
                        rounded = 1;
                    grpItem.Stake = rounded * process.env.NEXT_PUBLIC_StakeInnerMod0Combination;
                }
                if (grpItem.Stake < minInnerStakeAllowed) {
                    //new Stake Value
                    var divValueItemMin = minInnerStakeAllowed / process.env.NEXT_PUBLIC_StakeInnerMod0Combination;
                    var roundedMin = Math.ceil(divValueItemMin);
                    if (roundedMin === 0)
                        roundedMin = 1;
                    grpItem.Stake = roundedMin * process.env.NEXT_PUBLIC_StakeInnerMod0Combination;
                }
            }
